...
export const Route = createFileRoute('/accounts')({
  beforeLoad: () => requireAuthBeforeLoad('/accounts'),
  ...
  return (
    <main className="page-wrap px-4 pb-24 pt-10 md:pb-8">
      <section className="island-shell rounded-2xl p-6">
        ...
        <p className="text-[var(--sea-ink-soft)]">Server-scoped count for user: {finance.accountsCount}</p>
        <button
          onClick={() => incrementFinanceCounter({ data: { resource: 'accountsCount' } })}
          className="mt-4 rounded-md border border-[var(--line)] px-3 py-2"
          type="button"
        >
          Add account (server mutation)
        </button>
      </section>
    </main>
  )
